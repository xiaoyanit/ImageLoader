...
ImageLoader is a simple library that makes it easy to download, display and cache remote images in Android apps. Download happens away from the UI thread and the images are cached with a two-level in-memory/SD card cache.
...
If you want ImageLoader to clean up the SD card, add the following code in the onCreate of the Application class:
...
Cached preview images is a feature designed for when you have a list of items with thumbnail images and you subsequently display a larger version of the same image on some user action. 
ImageLoader can take the small image from the cache (if available) and use it as the preview image whilst the large image loads.
There are two options for implementing this: configuring the image tag before calling the load method or configuring the ImageTagFactory.
...
If you don't want to wait for us to fix a bug or implement a new feature, you can contribute to the project. Fork the repo and submit a pull request with your changes.
