...
It does that away from the ui thread, and by caching images with two levels in memory or in the sdcard if available.

h2. Recent changes

h3. 1.5.5

Few bug fixes, new DirectLoader utility class to directly download images (do not use it in the main thread).
...
Improve concurrent loader, change sdcard image position to respect Android SDK guide lines and improved LruBitmapCache.
...
https://github.com/novoda/public-mvn-repo/raw/master/releases/com/novoda/imageloader/imageloader-core/1.5.5/imageloader-core-1.5.5.jar

h2. How to implement

The demo project is a good way to start. Import it in eclipse and check the task list to see where are all the TODO list.
If you are lazy these are the steps :

h3. Application

In the application add the following code that instantiates the image loader:
...
The normal image loader works with softreferences. With a system with low memory as android, the system reclaims space to often. This limits the performance of the cache.
...
By default the LruBitmapCache will take 25% of the free memory available for the cache. You can change that by using a different constructor:
...
Preview image works well when you have a list of items with small images chaced and then you go to the details to show a big image. 
...
The image loader will fetch the image from the cache if is still there or from the sdcard or will fetch the resource from the network in the worst case scenario.
...
Importing the projects with maven, should be easy and painless. 
