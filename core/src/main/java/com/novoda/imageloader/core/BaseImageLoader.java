...
public class BaseImageLoader implements ImageManager {
	...
	@Override
	public void loadFromCacheOnly(String url, Context activity, ImageView imageView) {
		try {
			if (cacheManager.hasImageInCache(url)) {
				Bitmap b = cacheManager.getImageFromCache(url);
				if (b != null) {
			...
			imageView.setImageBitmap(cacheManager.getDefaultImage());
		} catch (Throwable t) {
			Log.e(TAG, t.getMessage(), t);
		}
	}
	...
	private ImageCache initCache(Context context, Settings settings) {
		...
		cache.setDefaultImage(bitmapUtil.decodeDefaultAndScaleBitmap(context,
		    settings));
		cache.setNotFoundImage(bitmapUtil.decodeNotFoundAndScaleBitmap(context,
		    settings));
		return cache;
	}
